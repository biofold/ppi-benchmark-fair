...
This document describes the **Bioschemas-based metadata model** and the **outputs generated by the five scripts**. The pipeline produces FAIR, machine-readable JSON-LD metadata for the *ELIXIR 3D-BioInfo Benchmark for Protein–Protein Interfaces*.
...
Each one of the script performs a specific transformation step in the pipeline. All other elements described in this document (JSON files, HTML files, FAIR packages) are **generated outputs**, not standalone scripts.


### Base Dataset and Interface Metadata Generation (ppi_benchmark_fair.py)
...
*This README documents the outputs generated by the five scripts and their Bioschemas-compliant metadata model.*
