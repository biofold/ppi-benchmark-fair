# PPI Benchmark FAIR Metadata Pipeline

This document describes the **Bioschemas-based metadata model** and the **outputs generated by the four scripts** contained in `all_scripts.py`. The pipeline produces FAIR, machine-readable JSON-LD metadata for the *ELIXIR 3D-BioInfo Benchmark for Protein–Protein Interfaces*.
...
The file `all_scripts.py` contains **five scripts**. Each script performs a specific transformation step in the pipeline. All other elements described in this document (JSON files, HTML files, FAIR packages) are **generated outputs**, not standalone scripts.
...
*This README documents the outputs generated by the scripts in `all_scripts.py` and their Bioschemas-compliant metadata model.*
